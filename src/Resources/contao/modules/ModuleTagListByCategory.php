...
class ModuleTagListByCategory extends Module
{
	...
	protected function getEventsForEventTags($tagids)
	{
		...
		if (count($tagids))
		{
			$objEvents = Database::getInstance()->prepare("SELECT *, (SELECT name FROM tl_user WHERE id=author) author FROM tl_calendar_events WHERE id IN (" . implode(",",$tagids) . ") " . (!BE_USER_LOGGED_IN ? " AND (start='' OR start<$time) AND (stop='' OR stop>$time) AND published=1" : "") . " ORDER BY startTime")
	...
	protected function getPagesForArticleTags($tags)
	{
		...
		{
			$time = time();
			$arrArticles = Database::getInstance()->prepare("SELECT id FROM tl_article WHERE id IN (" . implode(',',$tags) . ") AND pid IN (" . implode(',',$this->arrPages) . ") " . (!BE_USER_LOGGED_IN ? " AND (start='' OR start<?) AND (stop='' OR stop>?) AND published=1" : "") . " ORDER BY sorting")
	...
	protected function getArticlesForArticleTags($tags)
	{
		...
		{
			$time = time();
			$articles = Database::getInstance()->prepare("SELECT DISTINCT id, tl_article.* FROM tl_article WHERE id IN (" . implode(',',$tags) . ") AND pid IN (" . implode(',',$this->arrPages) . ") " . (!BE_USER_LOGGED_IN ? " AND (start='' OR start<?) AND (stop='' OR stop>?) AND published=1" : "") . " ORDER BY sorting")
				->execute($time, $time)
				->fetchAllAssoc();
		}
		return $articles;
	...
	protected function getPagesForContentTags($tags)
	{
		...
		if (count($this->arrPages) && count($tags))
		{
			...
			if (count($arrArticles))
			{
				$arrContentElements = Database::getInstance()->prepare("SELECT id FROM tl_content WHERE  id IN (" . implode(',',$tags) . ") AND pid IN (" . implode(',',$arrArticles) . ") " . (!BE_USER_LOGGED_IN ? " AND invisible<>1" : "") . " ORDER BY sorting")
