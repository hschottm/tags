	...
	protected function showTags()
	{
		...
		foreach ($this->arrTags as $idx => $tag)
		{
			...
			if ($this->checkForArticleOnPage)
			{
				...
				$arrArticles = Database::getInstance()->prepare("SELECT id FROM tl_article WHERE pid = ?")
					->execute($objPage->id)->fetchEach('id');
				$arrTagIds = Database::getInstance()->prepare("SELECT tid FROM " . $this->tag_tagtable . " WHERE from_table = ? AND tag = ?")
			...
			if ($this->checkForContentElementOnPage)
			{
				...
				{
					$arrCE = Database::getInstance()->prepare("SELECT id FROM tl_content WHERE pid IN (" . implode(",", $arrArticles) . ")")
		...
		$GLOBALS['TL_JAVASCRIPT'][] = 'bundles/hschottmtags/js/tagcloud.js';
		if (!empty($pageObj))
		{
